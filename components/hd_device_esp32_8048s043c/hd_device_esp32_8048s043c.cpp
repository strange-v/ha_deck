...
void HaDeckDevice::setup() {
    ...
    disp_drv.rotated = 0; // LV_DISP_ROT_0
    ...
    lv_obj_set_style_bg_opa(background, LV_OPA_COVER);
    lv_obj_add_style(background, LV_PART_MAIN, &lv_style_pretty);
    lv_obj_set_style_bg_color(background, lv_color_make(0, 0, 255), lv_color_make(255, 0, 0));
    lv_obj_set_style_bg_grad_color(background, lv_color_make(0, 0, 0), lv_color_make(255, 255, 255));
    lv_obj_set_style_bg_main_stop(background, 128);
    lv_obj_set_style_bg_grad_stop(background, 128);
   ...
   ESP_LOGCONFIG(TAG, "Free memory: %d bytes", lvgl. esp_get_free_heap_size());
}
